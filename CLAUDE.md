...
## ディレクトリ構造
- `src/questions/*.js` — 15分野の問題テンプレート（編集はここ。**questions.js を直接編集しない**）
  - 番号と分野: 01推論 / 02場合の数・確率 / 03集合 / 04損益算 / 05速度算 / 06仕事算 / 07濃度算 / 08割合・比 / 09図表 / 10順列・組合せ / 11四則逆算 / 12語句の関係 / 13規則性・方角 / 14整数の性質 / 15操作と手順
  - `_profile.js` — 出題プロファイル（どの分野を・どの言語で出すか）。`spi` / `koumuin` / `en` の3つ
  - `_base.js` — 共有の道具。`UNIT_LABELS`（単位）・`CATEGORY_LABELS`（分野名）を言語ごとに引く
  - ⚠️ **新しい展開先を足すのに `generator.js` と `app.js` を1行も触らずに済むか**が設計の判定基準
...
2. **検証を必ずCIと同じ手順で実行する**。変更後は以下を実行して壊れていないことを確認する（必要に応じてITERATIONS=1000に増やす）:
   ...
   - `node test/wiring.spec.js`（検査の登録漏れ。新しい spec を足したら必ず）
   - `node test/pwa.spec.js`
   - `node test/deeplink.spec.js`
   - `node test/mutation-runner.js`
   - `node test/app.spec.js`
   - `node test/profile.spec.js`
   - `node test/english.spec.js`
   - `node test/ui-text.spec.js`
   - `node test/tools.spec.js`
