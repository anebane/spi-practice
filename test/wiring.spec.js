...
if (!fs.existsSync(ciPath)) {
  ...
  if (shards.length) {
    ...
    // ⚠️ find の一意性は全体で1回しか判定できない。分割すると各シャードは
    //    自分の担当ぶんしか見ないので、全件まとめて確かめる工程が要る。
    //    これが無いと「検査を足したことで既存変異の find が2箇所に一致し、
    //    その変異が黙って効かなくなる」を取り逃す（2026-08-29に実際に起きた）。
    if (!/--check-mutations/.test(yml)) {
      fail("変異定義の全件検査がCIに無い",
        "分割すると find の一意性を全体で判定する工程が無くなる。"
        + "変異が黙って効かなくなっても気づけない");
    }
  }
