 ...
 *
 * ⚠️ この検査の対象は affiliate.js の**アフィリエイト素材だけ**。
 *    ネットワーク広告（忍者AdMax 等の配信タグ）は対象外で、PR表記も要求しない。
 *
 *    【この判断の出どころ（2026-09-11）】
 *    矢野さんの判断: 「アドネットワークであればPR表記不要。それが業界慣習」
 *    ⚠️ Claude が調べた範囲では、消費者庁の文書に
 *       「アドネットワーク経由のディスプレイ広告は表示不要」という**明示は無かった**
 *       （確認したのは「ステルスマーケティングに関するQ&A」。
 *        規制対象は広告主であり媒体ではない、とは書かれている）。
 *       業界慣習という根拠は Claude 側では検証できていない。
 *    → **判断の主体は矢野さん、根拠は業界慣習。**この行は「確認済みの事実」ではない。
 *       法令や運用が変わったとき、あるいは別の担当者が引き継ぐときは、
 *       ここを起点に再確認すること。無根拠に踏襲しない。
 */
