...
var Affiliate = (function () {
  ...
  var PROGRAMS = {
    ...
    // 素材001（クリック率3.65%）を使う。素材002「就職エージェントneo」は0.99%。
    // ※2026-09-03に管理画面で再確認した実測値。以前ここには「承認率5%以上／
    //   0.92%」と書いてあったが、管理画面のその列は承認率ではなくクリック率。
    // 確定率（申込のうち成果として承認される割合）は 35.71%。
    // 額面22,500円 × 35.71% ＝ 実効8,035円。面談型なので登録型より確定率が低い。
    //
    // ⚠️ アンカーは配布物の文言をそのまま使うこと。2026-09-03まで
    //    「最短1日で内定獲得可能！」と書かれていたが、配布物は「最短10日」。
    //    10倍に誇張した表示になっていた（優良誤認・素材改変の両面で問題）。
    ...
    neo: {
      ...
      active: true,
      anchor: "最短10日で内定獲得可能！就職エージェントneo",
      ...
      // 成果条件（2026-09-03にA8の管理画面で確認）:
      //   ・広告主新規 ・WEB申込後30日以内の面談実施完了
      //   ・対象は2026年卒・2027年卒 ← 2028年卒と書いていたのは誤りだった
      //   ・希望勤務地は東京/神奈川/千葉/埼玉/大阪/京都/兵庫
      // 対象外の人に申し込ませると全件否認になるので、学年と勤務地の両方を書く。
      note: "対象は2026年卒・2027年卒の学生で、希望勤務地が東京・神奈川・千葉・埼玉・大阪・京都・兵庫の方です。登録後にキャリアアドバイザーとの面談があります。",
      // ASPの管理画面で実際に確認した事実。test/affiliate.spec.js がこれと
      // anchor / note を突き合わせる。手で書いた文言が配布物や成果条件から
      // ずれても、目視では気づけないため（実際に「最短1日」「2028年卒」の
      // 2箇所がずれたまま本番で動いていた）。
      verified: {
        date: "2026-09-03",
        anchor: "最短10日で内定獲得可能！就職エージェントneo",  // 配布物の文言そのまま
        years: ["2026", "2027"]                                  // 成果条件の対象学年
      }
    },
