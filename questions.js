...
// ============================================================
// なぜ持つか（2026-09-07）:
//   英語圏（SHL型）への展開が決まり、単位を「表記そのもの」でテンプレートに
//   持たせたままだと、言語を足すたびに全テンプレートを書き換えることになる。
//   そこで単位を「キー → 言語ごとの表記」の表で引く形にした。
//   判定基準: **英語版を足すときに、テンプレート側を1本も書き換えずに済むか。**
//   英語版はこの表に en の列を足すだけでよい（テンプレートは触らない）。
//
// キーの決め方:
//   キーは**従来テンプレートが持っていた日本語の表記そのもの**。
//   抽象キー（"yen" など）に付け替えると全テンプレートの書き換えが要り、
//   移行そのものが事故の口になる。日本語の表記は単位の意味と1対1なので、
//   そのままキーとして使える。
//
// ⚠️ 3つの約束（test/generator.spec.js が見張る）:
//   1. ja の表記はキーと同一であること。ここがずれると既存の日本語の
//      出力が変わる。test/unit-labels-baseline.json が改修時点の表記を固定する
...
var WORD_PAIRS = {
  ...
  "行為の対象": [
    ...
    ["審判", "試合"], ["大工", "家屋"], ["調律師", "ピアノ"],
    ["翻訳者", "原文"], ["獣医", "動物"], ["保育士", "幼児"],
    ["学芸員", "展示品"], ["警察官", "容疑者"], ["消防士", "火災"],
    ["編集者", "原稿"], ["司書", "蔵書"], ["庭師", "庭木"],
    ["会計士", "帳簿"], ["美容師", "髪"], ["鑑定士", "骨董品"],
    ["写真家", "被写体"]
  ]
};
